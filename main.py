...
def open_file(event=None):
    ...
    open_path = askopenfilename(filetypes=[("Taipan File", "*.tai")])
    file_path = open_path
...
def save_file(event=None):
    global code, file_path
    if file_path == '':
        save_path = asksaveasfilename(defaultextension = ".tai", filetypes=[("Taipan File", "*.tai")])
        file_path = save_path
    else:
        save_path = file_path
...
status_bars = ttk.Label(window,text = "https://taipan.readthedocs.io/en/latest/ \t\t\t\t\t\t characters: 0 words: 0")
status_bars.pack(side = BOTTOM)
...
def change_word(event = None):
    ...
    if editor.edit_modified():
        ...
        status_bars.config(text = f"https://taipan.readthedocs.io/en/latest/ \t\t\t\t\t\t characters: {chararcter} words: {word}")
